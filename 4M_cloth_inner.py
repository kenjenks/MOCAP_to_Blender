...
def resolve_head_file_path(blend_file_path):
    ...
    if os.path.isabs(blend_file_path):
        if os.path.exists(blend_file_path):
            ...
            raise FileNotFoundError(f"ERROR: resolve_head_file_path Absolute path not found: {blend_file_path}")
    ...
    try:
        ...
        if os.path.exists(scene_relative_path):
            ...
            raise FileNotFoundError(f"ERROR: resolve_head_file_path Scene-relative path not found: {scene_relative_path}")

    except Exception as e:
        raise FileNotFoundError(f"ERROR: resolve_head_file_path Could not resolve scene-relative path {blend_file_path}: {e}")
...
def setup_imported_head(head_obj, armature_obj, garment_config, replaceable_config):
    ...
    if head_top_center and head_base_center:
        ...
        head_neck_radius = (get_bundle_radius(
            "CTRL_HEAD_TOP") + 0.05) / 2  # VIRTUAL_HEAD_BASE doesn't have radius, use default
        script_log(f"✓ Using midpoint between HEAD_TOP and VIRTUAL_HEAD_BASE: {head_neck_center}")
    else:
        # Fallback if virtual head base not available
        script_log("ERROR: VIRTUAL_HEAD_BASE not found")
        return None
...
def create_head(armature_obj, figure_name, garment_config, global_cloth_settings, neck_config=None):
    ...
    # =========================================================================
    # STEP 1: ENSURE NOSE CONTROL POINT EXISTS FIRST
    # =========================================================================
    script_log("STEP 1: Ensuring NOSE control point exists for safe constraint setup...")

    global mocap_data, frame_numbers

    if mocap_data and frame_numbers:
        first_frame = mocap_data.get(str(frame_numbers[0]), {})
        script_log(f"✓ Using frame {frame_numbers[0]} from loaded mocap_data")
    else:
        script_log("ERROR: create_head No mocap_data available")
        return None

    nose_obj = ensure_nose_control_point(first_frame)

    script_log(f"DEBUG: First frame keys: {list(first_frame.keys()) if first_frame else 'EMPTY'}")
    ...
    def create_procedural_head_safe(armature_obj, figure_name, garment_config, global_cloth_settings):
        ...
        try:
            ...
            # Get head-neck bundle from vertex bundles system
            ...
            bpy.ops.transform.resize(value=(1.1, 0.9, 1.2))
            ...
            # Parent the head to the armature
            head_obj.parent = armature_obj
            ...
            has_vertex_groups = len(head_obj.vertex_groups) > 0
            has_parent = head_obj.parent is not None
            ...
            script_log(f"✓ Using vertex bundle system with HEAD_TOP control point")
    ...
    def setup_head_bone_constraints(armature_obj, head_bone_name):
        """Setup constraints for head bone to follow control points"""
        script_log(f"Setting up head bone constraints for {head_bone_name}...")
        ...
        if not head_bone:
            script_log(f"ERROR: Head bone {head_bone_name} not found")
            return
        ...
        # Add constraint to follow HEAD_TOP control point
        head_top_obj = bpy.data.objects.get("CTRL_HEAD_TOP")
        if head_top_obj:
            copy_loc_constraint = head_bone.constraints.new(type='COPY_LOCATION')
            copy_loc_constraint.target = head_top_obj
            copy_loc_constraint.use_offset = False
            script_log(f"✓ Added COPY_LOCATION constraint to HEAD_TOP control point")
        else:
            script_log("ERROR: CTRL_HEAD_TOP control point not found")
            return None

        # Add rotation constraint to follow head orientation
        nose_obj = bpy.data.objects.get("CTRL_NOSE")
        if nose_obj:
            damped_track = head_bone.constraints.new(type='DAMPED_TRACK')
            damped_track.target = nose_obj
            damped_track.track_axis = 'TRACK_Z'
            script_log(f"✓ Added DAMPED_TRACK constraint to NOSE control point")
    ...
    if replaceable_config.get("export_template", False):
        ...
        try:
            ...
            scene_paths = get_scene_paths(args.show, args.scene)
            scene_dir = os.path.dirname(scene_paths["output_pose_data"])
            absolute_export_path = os.path.join(scene_dir, export_path)
        except:
            absolute_export_path = export_path

        success = save_hero_head_template(procedural_head, absolute_export_path, armature_obj, garment_config)
        if success:
            script_log(f"✓ Successfully exported head template to: {absolute_export_path}")
        else:
            script_log("✗ Failed to export head template")
    ...
    if replaceable_config.get("enabled", False):
        ...
        try:
            # Try to load replaceable head
            imported_head = load_replaceable_head(blend_file_path, figure_name, armature_obj)
            if imported_head:
                script_log("✓ Successfully loaded replaceable head")

                # Remove procedural head since we're using replaceable one
                bpy.data.objects.remove(procedural_head, do_unlink=True)

                # Setup the imported head
                setup_imported_head_with_bundles(imported_head, armature_obj, garment_config, replaceable_config)
                return imported_head
            else:
                script_log("✗ Failed to load replaceable head")
        except Exception as e:
            script_log(f"✗ Error loading replaceable head: {e}")

        # Fallback to procedural if replaceable loading fails
        if garment_config.get("fallback_to_procedural", True):
...
def create_procedural_head(armature_obj, figure_name, garment_config, global_cloth_settings, neck_config=None):
    ...
    try:
        ...
        # Get head-neck bundle diameter from vertex bundles system - USING NECK CONTROL POINT
        head_neck_diameter = garment_configs.get("diameter_neck", 0.16)
...
def create_cloth_garments(armature_obj, figure_name):
    ...
    # CREATE COAT TORSO (with physics flag check)
